                    ...
                    <ion-card>
                        <form @submit.prevent="submitForm">
                            ...
                            <ion-card-content>
                                <ion-row class="my-1">
                                    <ion-col>
                                        <ion-input class="form-control textfield" placeholder="New Username"
                                            style="color: white;" v-model="form.username" required></ion-input>
                                    </ion-col>
                                </ion-row>
                                <ion-row>
...
export default defineComponent({
    ...
    methods: {
        async submitForm() {
            this.isloading = true;
            try {
                const response = await axios.post(`${this.apiUrl}/api/user/update_username`, this.form);
                if (response.data.updated == true) {
                    ...
                    const alert = await alertController.create({
                        header: 'Error',
                        message: 'Unable to update new username',
                        buttons: ['OK'],
                    });
                    await alert.present();
                    this.isloading = false;
